...
public class UserService {
    ...
    @Autowired
    public UserService(PasswordEncoder passwordEncoder, UserRepository userRepository, RoleService roleService, RefreshTokenService refreshTokenService) {
	 ...
	 * LogOutRequest logOutRequest) { String deviceId =
	 * logOutRequest.getDeviceInfo().getDeviceId(); User user =
	 * userRepository.findByUserId(currentUser.getId()) .filter(device ->
	 * device.getDeviceId().equals(deviceId)) .orElseThrow(() -> new
	 * UserLogoutException(logOutRequest.getDeviceInfo().getDeviceId(),
