...
public class AuthService {
	 ...
	 * createAndPersistRefreshTokenForDevice(Authentication authentication,
	 * LoginRequest loginRequest) { User currentUser = (User)
	 * authentication.getPrincipal();
	 * userService.findByUserId(currentUser.getId()).map(User::getRefreshToken).map(
	 * RefreshToken::getId) .ifPresent(refreshTokenService::deleteById);
	 * 
	 * RefreshToken refreshToken = refreshTokenService.createRefreshToken();
	 * userDevice.setUser(currentUser); userDevice.setRefreshToken(refreshToken);
	 * refreshToken.setUserDevice(user); refreshToken =
	 * refreshTokenService.save(refreshToken); return
...
//	}
	...
	public Optional<Authentication> refreshJwtToken(@Valid TokenRefreshRequest tokenRefreshRequest) {
